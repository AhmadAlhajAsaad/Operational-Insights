...
| BR-7  | Ontbrekende person*id wordt auto-gegenereerd: `AUTO*{email*prefix}`of`AUTO*{UUID}`                     |
| BR-8  | Ontbrekende person*email wordt auto-gegenereerd: `unknown*{person_id}@placeholder.local`               |
| BR-9  | Ongeldige email formaten worden GEACCEPTEERD zonder validatie                                          |
| BR-10 | Ontbrekende org*id wordt auto-gegenereerd: `ORG*{name}`of`ORG\_{UUID}`                                 |
